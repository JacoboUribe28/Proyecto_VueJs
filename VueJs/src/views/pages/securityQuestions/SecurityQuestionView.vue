...
const questionId = route.params.id ? Number(route.params.id) : undefined;
const question = ref<{ name: string; description: string } | null>(null);
  ...
  <div class="min-h-screen bg-gray-100 p-6 flex items-center justify-center">
    <div class="w-full max-w-2xl bg-white shadow-lg rounded-lg p-8 space-y-10">
      <h1 class="text-4xl font-extrabold text-gray-800 text-center border-b pb-6">
        Detalles de la Pregunta de Seguridad
      </h1>

      <div v-if="question" class="space-y-10">
        <div class="bg-gradient-to-r from-blue-50 to-blue-100 p-6 rounded-lg shadow-md">
          ...
          <p class="text-gray-900 text-lg font-medium">{{ question.name }}</p>
        </div>
        <div class="bg-gradient-to-r from-green-50 to-green-100 p-6 rounded-lg shadow-md">
          ...
          <p class="text-gray-900 text-lg font-medium">{{ question.description }}</p>
        </div>
      </div>

      <div v-else class="text-center">
        <p class="text-gray-600 text-lg">Cargando detalles de la pregunta...</p>
      </div>

      <div class="text-center">
        <router-link to="/security" class="inline-block bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 transition text-lg font-semibold">
          Volver a la lista
        </router-link>
      </div>
    </div>
